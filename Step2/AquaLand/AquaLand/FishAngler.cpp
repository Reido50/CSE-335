...
{
	mFishImage = unique_ptr<Bitmap>(Bitmap::FromFile(FishAnglerImageName.c_str()));
	if (mFishImage->GetLastStatus() != Ok)
	{
		wstring msg(L"Failed to open ");
		msg += FishAnglerImageName;
		AfxMessageBox(msg.c_str());
	}
}

/**
 * Set the item location and handle caught fish location
 * \param x X location
 * \param y Y location
 */
void CFishAngler::SetLocation(double x, double y)
{
	...
	shared_ptr<CItem> caught = mCaughtFish.lock();

	if (caught != nullptr)
	{
		caught->SetLocationAngler(caught->GetX() + deltaX, caught->GetY() + deltaY);
	}
	
	CItem::SetLocation(x, y);

	CAquarium* aquarium = GetAquarium();
	auto newCatch = aquarium->CatchTest(GetX(), GetY());
	if (newCatch != caught && newCatch != nullptr)
	{
		mCaughtFish = newCatch;
	}
}
